...
// Command received via RF (and validated before processing)
...
void run(){
	...
	while(1){
		...
		if (state == INITIALIZE_STATE){
			...
			if (initialize() == HAL_OK){
				sprintf(log_message, "Initialization sucessful\n\r");
				HAL_UART_Transmit(&huart2,log_message,128,1000);
				memset(log_message, 0, 100);
				;
		...
		if (state == COEFFICENT_MODIFICATION_STATE){
			...
			sprintf(log_message, "Changed coeff %c of pid %c to %f\n\r", validated_command[2], validated_command[1], value);
			HAL_UART_Transmit(&huart2,log_message,128,1000);
			memset(log_message, 0, 100);
...
void command_handler(char received_command[NRF24L01P_PAYLOAD_LENGTH]){
	...
	switch(state){
			case FLYING_STATE:
				if ((received_command[1]=='0' )|(received_command[1]=='1')){
						...
						sprintf(log_message, "Received movement command : %s\n\r", validated_command);
						HAL_UART_Transmit(&huart2,log_message,128,1000);
						memset(log_message, 0, 100);
					}
...
HAL_StatusTypeDef initialize(void){
	...
	if (HAL_TIM_Base_Start(&htim5)==HAL_ERROR){
		return HAL_ERROR;
	}
	...
	htim3.Instance->PSC = 80-1;// Timer clock is 80 MHz
	...
	// Timers start
	if (HAL_TIM_PWM_Start(&htim8, TIM_CHANNEL_1)==HAL_ERROR){
		return HAL_ERROR;; // ultrasound trigger
	}
	if (HAL_TIM_IC_Start_IT(&htim4, TIM_CHANNEL_1)==HAL_ERROR){
		return HAL_ERROR; // ultrasound read
	}


	if (HAL_TIM_Base_Start_IT(&htim3)== HAL_ERROR){
		return HAL_ERROR;; // main loop
	}
